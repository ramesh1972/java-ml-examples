...
/**
 * Animal Classification - This project classifies animals in present in images
 *
...
//        MultiLayerNetwork network = new AlexNet(height, width, channels, numLabels, seed, iterations).init();
        ...
        StatsStorage statsStorage = new FileStatsStorage(new File("./", "ui-stats.dl4j"));
     ...
     // listeners
        network.setListeners(new StatsListener(statsStorage), new ScoreIterationListener(10), new EvaluativeListener(testIter, 1, InvocationType.ITERATION_END));
        ...
        for (int x=0; x< predictedClasses.length; x++) {
        	int labelIndex = testDataSet.getLabels().argMax(1).getInt(x);
        	String expectedResult = allClassLabels.get(labelIndex);    
        	String modelPrediction = allClassLabels.get(predictedClasses[x]);
            System.out.print("\nFor an example that is labeled " + expectedResult + " the model predicted " + modelPrediction + "\n\n");   	
        }
